...
function processNoorAbsenceFile(fileContent) {
  try {
    ...
    // 2. الحفاظ على القديم: قراءة السجلات الحالية لعدم تصفير الغائبين عن الملف الحالي
    // [تعديل] استخدام المتغيرات من Config.gs بدلاً من النصوص الثابتة
    const sheetsToRead = [ABSENCE_SHEET_INT_NAME, ABSENCE_SHEET_SEC_NAME];
    ...
    // 3. التحديث الذكي: قراءة ملف نور وتحديث الموجودين فيه فقط
    ...
    // [تعديل] استخدام المتغيرات من Config.gs
    saveToSheet_(ss, ABSENCE_SHEET_INT_NAME, listInt);
    saveToSheet_(ss, ABSENCE_SHEET_SEC_NAME, listSec);
...
function getAbsenceDashboardData() {
  try {
    ...
    // [تعديل] استخدام المتغيرات من Config.gs
    readSheet(ABSENCE_SHEET_INT_NAME);
    readSheet(ABSENCE_SHEET_SEC_NAME);
...
function updateStudentAbsence(data) {
  try {
    ...
    // [تعديل] استخدام المتغيرات من Config.gs
    if (!findInSheet(ABSENCE_SHEET_INT_NAME)) {
        findInSheet(ABSENCE_SHEET_SEC_NAME);
    }
